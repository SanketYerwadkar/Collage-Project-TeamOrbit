...
export const TaskList = ({ data, total }: TaskListProps) => {
  ...
  return (
    <div className="col-span-1 flex flex-col gap-y-4 rounded-2xl bg-white/20 backdrop-blur-lg border border-white/30 shadow-2xl p-3">
      <div style={{ height: '700px' }} className="rounded-lg bg-muted p-4 bg-[#E1F7F5]">
        <div className="flex items-center justify-between">
          <p className="text-lg font-semibold">Tasks ({total})</p>

          <Button className="bg-black" title="Create Task" variant="muted" size="icon" onClick={() => createTask()}>
          ...
          {data.map((task) => (
            <li key={task.$id}>
              <Link href={`/workspaces/${workspaceId}/tasks/${task.$id}`}>
                <Card className="rounded-lg shadow-none transition ">
        ...
        <Button
          variant="muted"
          className="mt-4 w-full bg-green-400 text-black-600   backdrop-blur-lg border border-white/30 shadow-2xl p-3"
          asChild
        >
...
export const ProjectList = ({ data, total }: ProjectListProps) => {
  ...
  return (
    <div className="col-span-1 flex flex-col gap-y-4 rounded-2xl bg-white/20 backdrop-blur-lg border border-white/30 shadow-2xl p-3">
      <div style={{ height: '700px' }} className="rounded-lg border bg-[#FDFAF6] p-6 ">
        <div className="flex items-center justify-between">
          <p className="text-lg font-semibold">Projects({total})</p>

          <Button className="bg-black " title="Create Project" variant="secondary" size="icon" onClick={createProject}>
            <PlusIcon className="size-4 text-neutral-400 " />
          </Button>
        </div>
            ...
            <li key={project.$id}>
              <Link href={`/workspaces/${workspaceId}/projects/${project.$id}`}>
                <Card className="rounded-lg shadow-none transition hover:opacity-75">
                  <CardContent className="flex items-center gap-x-2.5 p-4 rounded-2xl bg-white/20 backdrop-blur-lg border border-white/30 shadow-2xl p-2">
...
export const MemberList = ({ data, total }: MemberListProps) => {
  ...
  return (
    <div
      style={{ width: '1200px' }}
      className="col-span-1 flex flex-col gap-y-10 rounded-2xl bg-white/20 backdrop-blur-lg border border-white/30 shadow-2xl p-3"
    >
      <div className="rounded-lg border bg-white p-4">
        <div className="flex items-center justify-between">
          <p className="text-lg font-semibold">Members ({total})</p>

          <Button className="bg-black" title="Create Project" variant="secondary" size="icon" asChild>
        ...
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 ">
          {data.map((member) => (
            <li key={member.$id}>
              <Card className=" w-[325px]  overflow-hidden rounded-lg shadow-2xl rounded-2xl  backdrop-blur-lg border border-[#BDB395] shadow-2xl ">
